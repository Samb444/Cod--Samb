import React from 'react';
import { ArrowUp, ArrowUpRight } from 'lucide-react';
import './Footer.css';

const GithubIcon: React.FC<{ size?: number }> = ({ size = 16 }) => (
  <svg 
    width={size} 
    height={size} 
    viewBox="0 0 24 24" 
    fill="currentColor" 
    aria-hidden="true"
  >
    <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.53 1.032 1.53 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
  </svg>
);

const TiktokIcon: React.FC<{ size?: number }> = ({ size = 16 }) => (
  <svg 
    width={size} 
    height={size} 
    viewBox="0 0 24 24" 
    fill="currentColor" 
    aria-hidden="true"
  >
    <path d="M19.589 6.686a4.793 4.793 0 01-3.77-4.245V2h-3.445v13.672a2.896 2.896 0 01-5.209 1.641 2.908 2.908 0 012.313-4.545c.348 0 .684.062.997.175V9.435a6.37 6.37 0 00-.997-.08 6.35 6.35 0 00-6.352 6.352 6.353 6.353 0 006.352 6.352 6.31 6.31 0 005.424-3.134V9.824a8.21 8.21 0 004.687 1.458V7.837a4.847 4.847 0 01-.001-1.151z" />
  </svg>
);

const InstagramIcon: React.FC<{ size?: number }> = ({ size = 16 }) => (
  <svg 
    width={size} 
    height={size} 
    viewBox="0 0 24 24" 
    fill="none" 
    stroke="currentColor" 
    strokeWidth="2" 
    strokeLinecap="round" 
    strokeLinejoin="round" 
    aria-hidden="true"
  >
    <rect x="2" y="2" width="20" height="20" rx="5" ry="5" />
    <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
    <line x1="17.5" y1="6.5" x2="17.51" y2="6.5" />
  </svg>
);

export const Footer: React.FC = () => {
  const scrollToTop = () => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  return (
    <footer className="footer-root">
      <div className="container footer-inner">
        {/* Left: Brand & Factual Positioning */}
        <div className="footer-brand-col">
          <a href="#accueil" className="footer-brand" aria-label="Retour à l'accueil">
            <span className="brand-monogram">CS</span>
            <span className="brand-name">CODÉ SAMB</span>
          </a>
          <p className="footer-tagline">
            Étudiant en informatique · Développeur web · Créateur de solutions numériques
          </p>
          <span className="footer-location">Dakar, Sénégal · 2026</span>
        </div>

        {/* Right: Quick Links & Back to Top */}
        <div className="footer-links-col">
          <nav className="footer-nav" aria-label="Navigation secondaire de pied de page">
            <a href="#contact" className="footer-link">
              <span>Contact</span>
            </a>
            <a 
              href="https://github.com/Samb444" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="footer-link"
              aria-label="GitHub de Codé Samb (ouvre dans un nouvel onglet)"
            >
              <GithubIcon size={14} />
              <span>GitHub</span>
              <ArrowUpRight size={12} className="footer-arrow" aria-hidden="true" />
            </a>
            <a 
              href="https://www.tiktok.com/@mouridedev" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="footer-link"
              aria-label="TikTok de Codé Samb (ouvre dans un nouvel onglet)"
            >
              <TiktokIcon size={14} />
              <span>TikTok</span>
              <ArrowUpRight size={12} className="footer-arrow" aria-hidden="true" />
            </a>
            <a 
              href="https://www.instagram.com/begue_borom_touba" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="footer-link"
              aria-label="Instagram de Codé Samb (ouvre dans un nouvel onglet)"
            >
              <InstagramIcon size={14} />
              <span>Instagram</span>
              <ArrowUpRight size={12} className="footer-arrow" aria-hidden="true" />
            </a>
          </nav>

          <button 
            type="button" 
            onClick={scrollToTop} 
            className="footer-back-to-top"
            aria-label="Remonter en haut de la page"
            title="Remonter en haut de la page"
          >
            <span>Haut de page</span>
            <ArrowUp size={14} aria-hidden="true" />
          </button>
        </div>
      </div>
    </footer>
  );
};
